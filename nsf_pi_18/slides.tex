...
\title[Gradual Typing]{Challenges and Progress Toward Efficient Gradual Typing}
...
\includegraphics[height=1.1in]{andre} 
\end{minipage}
&
...
&
\begin{minipage}{0.25\textwidth}
\includegraphics[height=1.1in]{deyaa}
\end{minipage}
\end{tabular}
...
\begin{itemize}
\item Background on Gradual Typing
...
%===============================================================================
\frame{
\frametitle{The Buggy Example Revisited}
...
\begin{stack}
  \kw{let} \\
  ~~~~ f = \lam{y\of\Int} \kw{1} + y \\
  ~~~~ h = \lam{g\of\dyn} (g : \dyn \cast{} \dyn\tu\dyn) \app (\kw{true} : \Bool \cast{} \dyn)\\
  \kw{in} \\
  ~~~~ h \app (f : \Int\tu\Int \cast{} \dyn)
\end{stack} \\
\reduce^{*} \\
...
\begin{itemize}
\item Background on Gradual Typing
\item \textbf{Challenges to Efficiency}
\item Space-efficient Coercions
\item Monotonic Coercions
\item Performance Comparison
\end{itemize}
...
\[
\begin{stack}
\Letrec \mathit{even}(n \of \Int) : \rd{\dyn} = \\
~~~~ \If n = 0 \Then \True\\
~~~ \Else \mathit{odd}(n - 1) \\
\\
\Letrec \mathit{odd}(n \of \Int) : \Bool = \\
~~~~ \If n = 0 \Then \False\\
~~~ \Else \mathit{even}(n - 1) 
\end{stack}
\]
...
\begin{stack}
\Letrec \mathit{even}(n \of \Int) : \rd{\dyn} = \\
~~~~ \If n = 0 \Then (\True : \rd{ \Bool \cast{} \dyn}) \\
~~~ \Else (\mathit{odd}(n - 1) : \rd{ \Bool \cast{} \dyn})\\
\\
\Letrec \mathit{odd}(n \of \Int) : \Bool = \\
~~~~ \If n = 0 \Then \False \\
~~~ \Else (\mathit{even}(n - 1) : \rd{ \dyn \cast{} \Bool}) 
\end{stack}
\]
...
\begin{itemize}
\item An ahead-of-time compiler for gradual typing.

\item The input is a minimal functional language, output is C
  (e.g. no continuations).

\item Benchmarking to-date is limited due to amount of language
  support.
...
\includegraphics[width=4in]{plots/casts-or-coercions/quicksort_Proxied_Specialized.png}
...
\includegraphics[width=4in]{plots/casts-or-coercions/lin_fft_Proxied_Specialized.png}
...
\includegraphics[width=4in]{plots/casts-or-coercions/lin_n_body_Proxied_Specialized.png}
...
\includegraphics[width=4in]{plots/monotonic-v-coercions/lin_matmult_Coercions_Specialized_Lazy.png}
...
\includegraphics[width=4in]{plots/monotonic-v-coercions/lin_blackscholes_Coercions_Specialized_Lazy.png}
...
\item Gradual typing can be efficient!
  \footnote{Caveat: on the 8 benchmarks we have working so far.}
